...
def sample_data(data, size):
    ...
    sample = (
        ...
        .sample(cluster_sample, replace=True)["report_body"]
        .fillna("")
        .tolist()
    )
    return sample
...
ner_name = "/home/hwatkins/Desktop/neurollm/models/ner"
...
nottoolong = df.loc[
    (df["report_body"].str.len() < 1000) & (df["report_body"].str.len() > 30)
]
...
def even_sample(df, N):
    ...
    return sample_df["report_body"].tolist()


# narratives = sample_data(nottoolong, 500)
narratives = even_sample(nottoolong, 200)
print("sample: ", narratives[:3])
ner_output = neuro_pipeline(narratives)
no_musc = []
has_musc = lambda x: any([i["entity_group"] == "pathology_musculoskeletal" for i in x])
for out, text in zip(ner_output, narratives):
    if not has_musc(out):
        datum = {"text": text, "ents": out}
        no_musc.append(datum)
...
srsly.write_jsonl("./data/prospective_domain_labels_KCH.jsonl", labels)
